...
3. 可观察结果：同一运输单在同一阶段窗口内最多保留一张活动处置单，关闭时必须具备结构化触发依据、责任依据和回执证据；本样例未提供实测数据，结果标记为预期可观察。
...
1. 现有问题事实 SF1：现有流程按单点阈值逐条派发，短时波动与持续异常被等价处理，会在同一运输阶段形成重复任务。
2. 现有问题事实 SF2：温度、轨迹和现场动作已有采集记录，但缺少贯穿判定、派发和回执的统一处置对象。
3. 使用场景：冷链运输在途阶段发生连续温度偏离，需要在不改变设备采集频率的前提下完成异常识别、责任派发和回执闭环。
4. 系统边界：运输监测业务域提供事件；事件汇聚、异常处置和回执审计分别由独立系统承担；现场执行端只接收任务并提交证据。
5. 必要约束 SF3：方案不能改变设备采集频率，且需要复用既有工单接口，因此窗口聚合和证据校验必须在服务端完成。

### 行业内哪些竞争对手的业务、产品和本方案相关？请列出竞争对手的名称和相关业务、产品的名称（如有多个请一并列出）

待检索：当前合成输入未包含可核验的竞品材料，不为满足数量要求补写竞品能力。
...
1. **单点阈值逐条派发**：依据 SF1，既有方案每次越界都可形成独立任务，无法保证同一阶段窗口只保留一张活动处置单。
2. **判定记录与回执分开保存**：依据 SF2，既有方案缺少统一处置对象，关闭任务时无法直接核对触发依据、责任依据和现场证据是否属于同一次处置。
...
1. 窗口可由固定阶段参数替换为基于近期波动范围的自适应参数，但仍必须遵守阶段边界和单窗口单活动处置单约束。
2. 责任域可由静态映射替换为带人工确认的候选责任集，但确认结果仍需写回同一处置单并参与回执校验。

### 技术效果总结

1. **T1（对应 I1）**：原问题：同一阶段的多次越界形成重复任务。采用机制：阶段化异常窗口聚合同一窗口内相关事件，并约束只保留一张活动处置单。可观察结果：按运输单和阶段窗口统计的活动处置单数量不超过一。验证状态：`expected_observable`；验证证据来源：无。
2. **T2（对应 I2）**：原问题：判定依据、责任记录和回执材料分散。采用机制：责任域映射与回执证据约束随同一处置单传递并在关闭前校验。可观察结果：每张已关闭处置单均包含触发依据、责任依据和规定证据字段。验证状态：`expected_observable`；验证证据来源：无。

### 提炼本方案的关键技术创新点

1. **I1 阶段窗口驱动处置对象形成**：核心机制：按运输阶段维护异常窗口，并把窗口结果固化为分层处置单。必要约束：不改变设备采集频率，且同一窗口只保留一张活动处置单。实质差异：处置对象从单条越界记录变为受阶段边界约束的事件集合。落点为 S2、S3、S4 和 D2；对应效果为 T1。
2. **I2 责任与证据同对象闭环**：核心机制：把责任域映射结果和回执证据约束写入同一处置单。必要约束：复用既有工单接口，且关闭前必须通过证据校验。实质差异：判定、责任和回执不再由分散记录事后拼接。落点为 S5、S6、S7、E3、E4 和 E5；对应效果为 T2。
