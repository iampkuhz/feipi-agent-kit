...
for fixture in "${v1_fixtures[@]}"; do
  "$NODE_BIN" "$SKILL_DIR/scripts/validate_slide_ir.js" "$SKILL_DIR/fixtures/$fixture.slide-ir.json" --json > /dev/null
done

v2_fixtures=(layered-architecture solution-comparison multi-party-flow)
for fixture in "${v2_fixtures[@]}"; do
  "$NODE_BIN" "$SKILL_DIR/scripts/validate_slide_ir.js" "$SKILL_DIR/fixtures/acceptance/$fixture.slide-ir.v2.json" --json > /dev/null
done

"$NODE_BIN" "$SKILL_DIR/tests/p0/run.js"

"$NODE_BIN" "$SKILL_DIR/scripts/run_benchmarks.js" --dry-run --full --json > "$TMP_ROOT/benchmarks.json"
...
"$NODE_BIN" "$SKILL_DIR/scripts/generate_pptx_pipeline.js" \
  "$SKILL_DIR/fixtures/acceptance/layered-architecture.slide-ir.v2.json" \
