...
description: 用于下载 YouTube 视频或音频并保存到本地目录，覆盖 dryrun、字幕提取、whisper.cpp 转写和代理/认证回退策略。在需要验证链接可下载、提取音频或沉淀 YouTube 视频素材时使用。
---
...
1. 用户一开始就给播放列表
- 处理：先确认是否真的要批量下载，再展开执行。

2. 直连失败或 bot 检测
- 处理：优先让脚本走现有代理/认证回退链路，不手工改复杂参数。

3. 字幕模式没有产出
- 处理：先判断是否字幕缺失，再改走 `whisper`。
