...
VALIDATE_SCRIPT="$RUNTIME_DIR/validate.sh"
INSTALL_SCRIPT="$RUNTIME_DIR/install_deps.sh"
DOWNLOAD_SCRIPT="$RUNTIME_DIR/download_video.sh"
EXTRACT_SCRIPT="$RUNTIME_DIR/extract_video_text.sh"
SUMMARY_SCRIPT="$RUNTIME_DIR/render_summary_prompt.sh"
BACKGROUND_SCRIPT="$RUNTIME_DIR/render_background_prompt.sh"
...
# 加载真实 lib 函数，而非重复正则
YT_RETRY_POLICY_LIB="$RUNTIME_DIR/lib/youtube_retry_policy.sh"
...
TRANSCRIBE_SCRIPT="$RUNTIME_DIR/lib/whispercpp_transcribe.sh"
...
YOUTUBE_SCRIPT="$RUNTIME_DIR/download_youtube.sh"
...
BILIBILI_SCRIPT="$RUNTIME_DIR/download_bilibili.sh"
...
YT_COMMON_LIB="$RUNTIME_DIR/lib/yt_dlp_common.sh"
...
MOCK_DIR="$SCRIPT_DIR/fixtures/transcripts"
MOCK_OUTPUT_DIR="$SCRIPT_DIR/fixtures/summary_outputs"
LINT_PROMPT="$RUNTIME_DIR/lint_prompt_contract.sh"
LINT_RESULT="$RUNTIME_DIR/lint_summary_result.sh"
...
if [[ -f "$MOCK_DIR/long_transcript.txt" ]]; then
  SEGMENT_SCRIPT="$RUNTIME_DIR/render_segment_summary_prompts.sh"
  FINAL_SCRIPT="$RUNTIME_DIR/render_final_summary_from_segments_prompt.sh"
