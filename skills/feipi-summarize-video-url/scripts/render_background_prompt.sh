...
2) 在该章节内必须使用以下小标题，且顺序固定：
   - ### 背景知识补充（约2/3）
   - ### 关键影响（约1/3）
3) “背景知识补充”必须基于视频外公开资料（历史背景、制度沿革、新闻原文、术语解释），禁止仅按视频时间线复述或改写转写内容。
4) “背景知识补充”必须先列出 3-6 个“关键术语/人物/机构/事件”（来自视频关键词，列表形式），随后按清单逐条补充外部背景，每条必须包含：
   - 是什么（定义/角色） + 至少 1 个外部历史/新闻事实（含日期） + 与视频观点的关系。
5) “背景知识补充”末尾必须追加“来源清单”（列表形式），每条至少包含：
   - 来源名/机构 + 日期 + 原文标题/文件名（不得只写“网络资料/媒体报道”）。
   - 来源不少于 3 条，优先官方/主流媒体/研究机构/学术或政策文件。
   - 至少包含 1-2 条“新闻原文或原始文件”来源。
6) “关键影响”每条都要写清：
...
8) 禁止重复第一轮的同义改写；每个列表项都必须新增信息价值。
9) 禁止套用固定条款/模板术语（如无关条款号或政策名），必须围绕本次视频关键词展开。
10) 禁止输出“关键不确定性”“后续观察点是否有诉讼”等空洞小节。
11) 禁止出现“转写误差、不确定、信息不足、仅供参考”等质量豁免表达。
