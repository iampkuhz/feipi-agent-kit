...
description: 用于根据视频 URL 调用来源技能提取带时间戳文本，并生成两阶段总结请求包与交付约束。在需要先交付结构化摘要、再补充背景与影响分析时使用。
---
...
1. 用户给出 YouTube 或 Bilibili URL，希望先拿到结构化摘要，再拿到背景分析。
2. 用户需要带时间锚点的总结，而不是只有纯结论。
3. 用户希望复用来源技能完成提取，不手写下载或转写流程。
...
- 转写文本与产物文件名不包含空格。
- `summary_request.md`、`summary_result.md`、`background_request.md`、`background_result.md` 均非空；为空视为失败需重跑。

## 常见失败与修复

1. 执行方只生成请求包，不继续给结果
- 处理：按强约束补齐两阶段实际交付，不能停在请求包。

2. 背景分析只是复述视频
- 处理：回到第二阶段要求，补视频外公开资料与来源清单。

3. 选档不符合用户意图
- 处理：先看原始指令里是否有高质量信号，再校验 `whisper_profile` 与 `selection_reason`。
