...
- 背景必须覆盖本次视频中的关键术语/人物/机构/事件（至少 3-6 个），先列清单再逐条补充。
...
- 第二次交付：输出“背景知识补充（约 2/3）+ 关键影响（约 1/3）”，其中背景必须覆盖本次视频关键词，避免重复第一次内容。
...
- 根据 `background_request.md` 直接产出第二次结果（`background_result.md`），紧接第一次交付输出。
...
- 第二次请求包明确要求“背景知识约 2/3 + 关键影响约 1/3”，并基于本次视频关键词展开。
- 转写文本与产物文件名不包含空格。
