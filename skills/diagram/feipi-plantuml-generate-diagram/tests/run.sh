...
run_validate() {
  ...
  rm -rf "$out_dir"
  if ! bash "$RUNTIME_DIR/validate_package.sh" "$@" --out-dir "$out_dir" --server-url "$RENDERER_URL"; then
...
# =============================================================================
echo "=== Step 1: 结构校验 ==="
bash "$RUNTIME_DIR/validate.sh" "$SKILL_DIR" >/dev/null && pass "结构校验" || fail "结构校验"
...
if ! python3 "$RUNTIME_DIR/run_mindmap.py" --brief "$MINDMAP_BRIEF" --out-dir "$MINDMAP_OUT" > "$MINDMAP_OUT/cli-result.json"; then
...
if bash "$RUNTIME_DIR/validate_package.sh" \
...
if python3 "$RUNTIME_DIR/check_coverage.py" --type sequence --brief "$SEQ_S_BRIEF" \
...
if python3 "$RUNTIME_DIR/check_coverage.py" --type component --brief "$COMPONENT_BRIEF" \
...
if bash "$RUNTIME_DIR/lint_layout.sh" --type sequence "$PROCESS_S_AUTONUMBER" "$SEQ_S_BRIEF" >/dev/null 2>&1; then
...
if [[ "$HASH_STATUS" == "success" ]]; then
  if python3 "$RUNTIME_DIR/verify_package.py" "$HASH_OUT" >/dev/null 2>&1; then
...
cp "$RUNTIME_DIR/validate_package.sh" "$MISSING_RENDER_ROOT/scripts/validate_package.sh"
cp -R "$RUNTIME_DIR/lib" "$MISSING_RENDER_ROOT/scripts/lib"
